        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-green-100 mt-1">Review and verify exhumation permit requests</p>
            </div>
            <div className="text-right">
              <p className="text-sm text-green-100">Cemetery Staff</p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-gradient-to-br from-green-500 to-green-600 text-white rounded-lg shadow-md p-6">
            ...
            <p className="text-xs text-green-100 mt-2">Awaiting review</p>
          </div>
          <div className="bg-gradient-to-br from-red-500 to-red-600 text-white rounded-lg shadow-md p-6">
            <p className="text-sm text-green-100 mb-1">Returned for Correction</p>
            <p className="text-4xl font-bold">{returnedPermits.length}</p>
            <p className="text-xs text-green-100 mt-2">Waiting for resubmission</p>
          </div>
        </div>
          ...
          {pendingPermits.length === 0 ? (
            ...
            <div className="space-y-4">
              {pendingPermits.map((permit) => (
                <div key={permit.id} className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow border-l-4 border-green-500">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="bg-green-50 p-3 rounded-lg">
                        <p className="text-sm text-green-900 font-medium mb-1">Reason for Exhumation:</p>
                        <p className="text-sm text-green-800">{permit.reasonForExhumation}</p>
                      </div>
                    ...
                    <Link href={`/services/cemetery/exhumation-permits/verification/${permit.id}`}>
                      <button className="ml-4 px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors font-medium">
                        Review →
                      </button>
                    </Link>
                  </div>
