...
export default async function EmployeeDashboard() {
  ...
  const [stats, permitStats, exhumationStats, cremationStats, certificateStats, permitsAwaitingCemetery] = await Promise.all([
    prisma.deathRegistration.groupBy({
      by: ['status'],
      _count: true
    }),
    prisma.burialPermit.groupBy({
      by: ['status'],
      _count: true
    }),
    prisma.exhumationPermit.groupBy({
      by: ['status'],
      _count: true
    }),
    prisma.cremationPermit.groupBy({
      by: ['status'],
      _count: true
    }),
    prisma.deathCertificateRequest.groupBy({
      by: ['status'],
      _count: true
    }),
    prisma.cemeteryPermitSubmission.count({
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-green-100 mt-1">Process death registrations and manage civil registry records</p>
            </div>
