...
export default function ParksAnalyticsPage() {
  ...
  const exportToCSV = useCallback(() => {
    ...
    const rows = [
      ...
      [], ["Location", "Count"],
      ...data.topLocations.map((l) => [l.location, l.count]),
  ...
  const exportToExcel = useCallback(() => {
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([
      ["Location", "Count"], ...data.topLocations.map((l) => [l.location, l.count]),
    ]), "Top Locations")
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h1 className="text-2xl font-bold">Parks & Recreation Analytics</h1>
            </div>
            <div className="flex gap-2">
              <button onClick={exportToCSV} className="px-3 py-1.5 bg-teal-700 hover:bg-teal-800 rounded text-sm font-medium transition-colors">CSV</button>
              <button onClick={exportToExcel} className="px-3 py-1.5 bg-teal-700 hover:bg-teal-800 rounded text-sm font-medium transition-colors">Excel</button>
              <button onClick={exportToPDF} className="px-3 py-1.5 bg-teal-700 hover:bg-teal-800 rounded text-sm font-medium transition-colors">PDF</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
          {[
            { label: "Total Requests", value: data.kpis.totalRequests, border: "border-teal-600" },
          ...
          {data.volumeTrends.every((v) => v.count === 0) ? (
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={data.volumeTrends}>
                ...
                <Tooltip />
                <Bar dataKey="count" name="Requests" fill="#0d9488" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          {/* Top Locations - Table or Bar Chart depending on count */}
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-lg font-bold text-gray-800 mb-4">Top Locations</h2>
            {data.topLocations.length === 0 ? (
              <p className="text-gray-500 text-center py-6">No location data available</p>
            ) : data.topLocations.length < 6 ? (
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    <th className="text-left py-2 text-sm font-semibold text-gray-600">Location</th>
                ...
                <tbody>
                  {data.topLocations.map((l) => (
                    <tr key={l.location} className="border-b border-gray-100">
                      <td className="py-2.5 text-sm text-gray-700">{l.location}</td>
                      <td className="py-2.5 text-sm font-semibold text-gray-800 text-right">{l.count}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={data.topLocations} layout="vertical">
                  ...
                  <Tooltip />
                  <Bar dataKey="count" name="Requests" fill="#0d9488" radius={[0, 4, 4, 0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </div>
