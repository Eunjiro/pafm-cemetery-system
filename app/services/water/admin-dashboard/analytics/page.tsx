...
export default function WaterAnalyticsPage() {
  ...
  const exportToCSV = useCallback(() => {
    ...
    const rows = [
      ...
      [], ["Barangay", "Count"],
      ...data.topBarangays.map((b) => [b.barangay, b.count]),
    ...
    a.download = `water-analytics-${new Date().toISOString().slice(0, 10)}.csv`; a.click()
  }, [data])
  ...
  const exportToExcel = useCallback(() => {
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([
      ["Barangay", "Count"], ...data.topBarangays.map((b) => [b.barangay, b.count]),
    ]), "Top Barangays")
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([
  ...
  const exportToPDF = useCallback(() => {
    if (!data) return
    const doc = new jsPDF()
    doc.setFontSize(18); doc.text("Water Services Analytics", 14, 22)
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-blue-600 text-white py-6 shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            <div>
              <Link href="/services/water/admin-dashboard" className="text-sm text-blue-200 hover:text-white mb-2 inline-block">← Back to Dashboard</Link>
              <h1 className="text-2xl font-bold">Water Services Analytics</h1>
            </div>
            <div className="flex gap-2">
              <button onClick={exportToCSV} className="px-3 py-1.5 bg-blue-700 hover:bg-blue-800 rounded text-sm font-medium transition-colors">CSV</button>
              <button onClick={exportToExcel} className="px-3 py-1.5 bg-blue-700 hover:bg-blue-800 rounded text-sm font-medium transition-colors">Excel</button>
              <button onClick={exportToPDF} className="px-3 py-1.5 bg-blue-700 hover:bg-blue-800 rounded text-sm font-medium transition-colors">PDF</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
          {[
            { label: "Total Requests", value: data.kpis.totalRequests, border: "border-blue-600" },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          {/* Top Barangays - Table or Bar Chart depending on count */}
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-lg font-bold text-gray-800 mb-4">Top Barangays</h2>
            {data.topBarangays.length === 0 ? (
              <p className="text-gray-500 text-center py-6">No barangay data available</p>
            ) : data.topBarangays.length < 6 ? (
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    <th className="text-left py-2 text-sm font-semibold text-gray-600">Barangay</th>
                    <th className="text-right py-2 text-sm font-semibold text-gray-600">Requests</th>
                  </tr>
                </thead>
                <tbody>
                  {data.topBarangays.map((b) => (
                    <tr key={b.barangay} className="border-b border-gray-100">
                      <td className="py-2.5 text-sm text-gray-700">{b.barangay}</td>
                      <td className="py-2.5 text-sm font-semibold text-gray-800 text-right">{b.count}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={data.topBarangays} layout="vertical">
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis type="number" allowDecimals={false} />
                  <YAxis type="category" dataKey="barangay" width={120} tick={{ fontSize: 11 }} />
                  <Tooltip />
                  <Bar dataKey="count" name="Requests" fill="#2563eb" radius={[0, 4, 4, 0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </div>
